import { CORE_SCHEMA, loadAll as parseYamlDocuments } from 'js-yaml'
import {
...
export const normalizeTransformOptions = (value) => {
  ...
  const languagesProvided = hasOwn(options, 'languages')
  if (languagesProvided && !Array.isArray(options.languages)) {
    throw new TypeError('options.languages must be an array when provided.')
  }
  const markRegState = hasMarks || languagesProvided
    ? getMarkRegStateForLanguages(
        languagesProvided ? options.languages : undefined,
      )
    : null
