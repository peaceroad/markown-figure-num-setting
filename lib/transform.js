...
const getMarkAnalysisState = (invocation, mark) => {
  ...
  const analysisOptions = invocation.options.analysisOptions
  analysisState = {
    generatedOptions:
      analysisOptions.preferredMark === mark
        ? analysisOptions
        : {
            markRegState: analysisOptions.markRegState,
            preferredMark: mark,
          },
...
const collectCaptionCandidates = (state, invocation, timeline) => {
  ...
  for (let tokenIndex = 0; tokenIndex < state.tokens.length; tokenIndex++) {
    ...
    const leadingAnalysis = analyzeCaptionStart(
      inline.content,
      invocation.options.analysisOptions,
    )
    ...
    const decision = analyzeCaptionParagraph(
      ...
      invocation.options.paragraphAnalysisOptions,
    )
